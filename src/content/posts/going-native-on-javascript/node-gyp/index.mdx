---
title: "Native Node.js Addons with node-addon-api"
published: 2023-09-29T00:00:00.000Z
description: >-
  Learn how to build native Node.js addons effortlessly using node-addon-api,
  from basic setup to basic OpenCV functionalities.
tags:
  - C++
  - NAPI
  - JavaScript
...
<Trivia emoji="🥸">
  I will actually be using `pnpm` as a personal preference. You can use `npm` or
  `yarn` as well. (Or even bun 🍞 if you are feeling adventurous!)
</Trivia>
  ...
  "targets": [
    {
      ...
      "sources": ["<!@(find addon/src -name *.cc)"],
      "include_dirs": [
...
```ts title="lib/binding.ts"
export function resize(inputPath: string, width: number, height: number): void;
```
